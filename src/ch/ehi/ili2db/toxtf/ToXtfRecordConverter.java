				...
				if (type instanceof CompositionType){
					...
					if(createEnumColAsItfCode){
			                if(classAttr==null) {
		                        ...
		                        Object rawValue=rs.getObject(valuei);
		                        valuei++;
		                        if(rawValue!=null){
		                            if(rawValue instanceof Number) {
		                                ...
		                                String valueText=((String)rawValue).trim();
		                                if(valueText.length()>0) {
		                                    try {
		                                        int value=Integer.parseInt(valueText);
		                                        iomObj.addattrvalue(attrName,mapItfCode2XtfCode((EnumerationType)type, value));
		                                    }catch(NumberFormatException ex) {
		                                        // Some backends persist enum columns as textual XTF values.
		                                        iomObj.addattrvalue(attrName,valueText);
		                                    }
		                                }
		                            }else {
		                                EhiLogger.logError("Object "+sqlid+": unexpected enum value type "+rawValue.getClass().getName()
		                                        +" for attribute "+classAttr);
		                            }
		                        }
			                }
					}else{
